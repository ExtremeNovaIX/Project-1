...
public class MemoryWriteService {
     ...
     */
    public List<MemoryArchiveDocument> saveEventGroup(String sessionId, List<ExtractedMemoryEventDTO> events) {
        return saveEventGroup(sessionId, events, List.of());
    }

    public List<MemoryArchiveDocument> saveEventGroup(String sessionId,
                                                      List<ExtractedMemoryEventDTO> events,
        ...
        for (int index = 0; index < events.size(); index++) {
            ExtractedMemoryEventDTO event = events.get(index);
     ...
     */
    public MemoryArchiveDocument saveNewMemory(String sessionId, ExtractedMemoryEventDTO event) {
        MemoryArchiveDocument archive = buildArchive(sessionId, event, null, null);
        return saveArchive(archive, "新事件写入");
    }
    ...
    private MemoryArchiveDocument saveArchive(MemoryArchiveDocument archive, String action) {
        ...
        try {
            MemoryArchiveDocument saved = archiveService.save(archive);
    ...
    private String resolveGroupTopic(List<ExtractedMemoryEventDTO> events) {
        if (events == null || events.isEmpty()) {
            return "";
        }
        ExtractedMemoryEventDTO headEvent = events.getFirst();
