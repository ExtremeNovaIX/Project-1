...
public class RecentWindowTraceService {
    ...
    public RecentWindowTraceService(MemoryArchiveMarkdownService archiveService) {
    ...
    public void writeTrace(Long rootArchiveId, ScoreTrace trace) {
        ...
        archiveService.findById(rootArchiveId).ifPresent(rootArchive -> {
            rootArchive.setEventGraphTrace(toMarkdown(trace));
            archiveService.save(rootArchive);
        });
    }
    ...
    private String toMarkdown(ScoreTrace trace) {
        ...
        if (winner == null) {
            ...
            body.append(label(winner.groupId(), winner.targetArchiveId()))
                    .append(" (最佳分=").append(formatDouble(winner.bestScore()))
                    .append(", 时间衰减=").append(formatDouble(winner.timeDecayFactor()))
                    .append(", 候选年龄小时=").append(winner.candidateAgeHours() == null ? "-" : formatDouble(winner.candidateAgeHours()))
                    .append(", idf01=").append(formatDouble(winner.normalizedIdfScore()))
                    .append(", 提升后支持度=").append(formatDouble(winner.boostedSupportScore()))
        ...
        if (trace.candidates().isEmpty()) {
            ...
            for (CandidateTrace candidate : trace.candidates().stream().limit(MARKDOWN_CANDIDATE_PREVIEW_LIMIT).toList()) {
                body.append("- 候选=")
                        ...
                        .append(", 时间衰减=").append(formatDouble(candidate.timeDecayFactor()))
                        .append(", 候选年龄小时=").append(candidate.candidateAgeHours() == null ? "-" : formatDouble(candidate.candidateAgeHours()))
                        .append(", idf01=").append(formatDouble(candidate.normalizedIdfScore()))
                        .append(", 提升后支持度=").append(formatDouble(candidate.boostedSupportScore()))
                        .append(", 最佳分=").append(formatDouble(candidate.bestScore()))
